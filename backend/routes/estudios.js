...
router.get("/pdf", auth(), async (req, res) => {
  try {
    ...
    const [[pac]] = await pool.query(
      "SELECT id, nombres, apellidos, fecha_nacimiento, dni FROM pacientes WHERE id=? AND clinica_id=?",
      [paciente_id, cid]
    );
    if (!pac) return res.status(404).json({ ok: false, msg: "Paciente no encontrado" });

    const [[clinica]] = await pool.query(
      "SELECT nombre, direccion, telefono FROM clinicas WHERE id=?",
      [cid]
    );
    ...
    let sql = `
      SELECT s.*, u.nombres AS med_nombres, u.apellidos AS med_apellidos
      FROM estudios_solicitudes s
      JOIN usuarios u ON u.id = s.medico_id
    ...
    if (estudios.length === 0) {
      ...
      estudios.forEach((s, i) => {
        ...
        const descH = doc.heightOfString(desc, { width: pageW - 30, lineGap: 1.5 });
        bodyY += descH + 10;
    ...
    // ── Firma ───────────────────────────────────────────────────────
    const minBodyY = tituloY + 150;
    let rowY = Math.max(bodyY + 10, minBodyY);
